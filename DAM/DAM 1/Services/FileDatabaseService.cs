...
public class FileDatabaseService
{
    ...
    public FileDatabaseService(string connString)
    {
    ...
    private void CreateDatabase()
    {
        using (var conn = new NpgsqlConnection(connString))
        {
            ...
            string insertDataInProductsQuery = @"
            ...
            using (var cmd = new NpgsqlCommand(insertDataInProductsQuery, conn))
            {
                cmd.ExecuteNonQuery();
            }

            string insertDataInTagsQuery = @"
            ...
            using (var cmd = new NpgsqlCommand(insertDataInTagsQuery, conn))
            {
                cmd.ExecuteNonQuery();
            }

            string insertDataInProductTagQuery = @"
            ...
            using (var cmd = new NpgsqlCommand(insertDataInProductTagQuery, conn))
            {
                cmd.ExecuteNonQuery();
            }
    ...
    /// <summary>
    /// Saves the Metadata in the database
    /// </summary>
    /// <param name="fileName"> The name of the file</param>
    /// <param name="filePath"> The path to the file</param>
    public void SaveFileMetadata(string fileName, string filePath)
    {
        using var conn = new NpgsqlConnection(connString);
        conn.Execute(@"
        INSERT INTO products (filename, dimensionSize, type, path, metadata, transformations)
        VALUES (@FileName, @DimensionSize, @Type, @Path, '{}'::jsonb, '{}'::jsonb)
        ON CONFLICT (filename) DO UPDATE 
        SET path = EXCLUDED.path;",
            new
            {
                FileName = fileName,
                DimensionSize = GetImageDimensions(filePath),
                Type = GetMimeType(Path.GetExtension(filePath)),
                Path = filePath
            });
    }
    ...
    /// <summary>
    /// Funktion that inserts a batch af data in the database
    /// </summary>
    /// <param name="relativePath"> For when you dont know the full path</param>
    public void InsertFilesFromDirectory(string relativePath)
    {
        string absolutePath = Path.GetFullPath(relativePath);

        if (!Directory.Exists(absolutePath))
        {
            Console.WriteLine($"Directory not found: {absolutePath}");
            return;
        }
        ...
        using (var conn = new NpgsqlConnection(connString))
        {
            ...
            foreach (var file in files)
            {
                ...
                string fileType = GetMimeType(fileInfo.Extension);
                string dimensionSize = GetImageDimensions(file);

                string query = @"
                        INSERT INTO products (filename, dimensionSize, type, path, metadata, transformations)
                        VALUES (@filename, @dimensionSize, @type, @path, @metadata, @transformations)
                        ON CONFLICT (filename) DO NOTHING;";

                using (var cmd = new NpgsqlCommand(query, conn))
                {
                    cmd.Parameters.AddWithValue("@filename", fileInfo.Name);
                    cmd.Parameters.AddWithValue("@dimensionSize", dimensionSize);
                    cmd.Parameters.AddWithValue("@type", fileType);
                    cmd.Parameters.AddWithValue("@path", Path.Combine("media", fileInfo.Name));
                    cmd.Parameters.Add("@metadata", NpgsqlTypes.NpgsqlDbType.Jsonb).Value = "{}";
                    cmd.Parameters.Add("@transformations", NpgsqlTypes.NpgsqlDbType.Jsonb).Value = "{}";
                    ...
                    {
                        Console.WriteLine($"Error inserting {fileInfo.Name}: {ex.Message}");
                    }
                }
            }
    ...
    /// <summary>
    /// Finds the dimansions of the image
    /// </summary>
    ...
    /// </summary>
    public class DamPictureProvider : IDamPictureProvider
    {
        private readonly string _connString;

        public DamPictureProvider(string connString)
        {
        ...
        public byte[] GetPicture(int productId)
        {
            ...
            var cmd = new NpgsqlCommand("SELECT image_data FROM products WHERE id = @id;", conn);
